...
public static class Site_Patch
{
    ...
    public static void ShouldRemoveMapNow_Postfix(Site __instance, ref bool __result)
    {
        ...
        if(__instance.parts.Any(p=> p.def == Thirst_Flavour_PackDefOf.MSS_Thirst_BarbCamp))
        {
            if (!__instance.Map.mapPawns.AnyColonistSpawned && !__instance.Map.PlayerPawnsForStoryteller.Any())
            {
                __result = true;
                return;
            }
